// ──────────────────────────────────────────────────────────
// Logger adapter — same pattern as frontend-developer
// ──────────────────────────────────────────────────────────
...
export function createChildLogger(name: string): ChildLogger {
  const log = (level: "info" | "warn" | "error", objOrMsg: unknown, msg?: string) => {
    if (typeof objOrMsg === "string") {
      logger[level](`[pm:${name}] ${objOrMsg}`);
    } else {
      logger[level](`[pm:${name}] ${msg ?? ""}`, objOrMsg as Record<string, unknown>);
    }
  };
