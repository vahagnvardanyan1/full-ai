...
function AgentNode({ data, selected }: NodeProps<Node<AgentNodeData>>) {
  ...
  // Card styles must stay inline for React Flow (dynamic values)
  ...
  return (
    <>
      ...
      <div style={card}>
        {/* GitHub push overlay */}
        {justPushed && (
          <div className="absolute inset-0 flex flex-col items-center justify-center gap-1.5 bg-black/65 backdrop-blur-[4px] rounded-[13px] z-10 animate-github-push-overlay">
            <svg width={28} height={28} viewBox="0 0 98 96" fill="#22c55e" className="animate-github-push-icon">
            ...
            <span className="text-[0.6rem] font-bold uppercase tracking-wider text-green-500">
              {pushLabel}
            </span>
          </div>
        )}
        ...
        <div className="flex items-center gap-2">
          ...
          <div className="flex-1 min-w-0">
            <div className="text-[0.8rem] font-semibold text-[var(--text)]">{label}</div>
            <div
              className="text-[0.6rem] font-medium uppercase tracking-wide"
              style={{ color: isError ? "var(--error)" : isWorking ? color : isDone ? "var(--success)" : "var(--text-muted)" }}
            >
        ...
        {/* Summary preview */}
        {isDone && summary && (
          <div className="text-[0.72rem] leading-snug text-[var(--summary-dim)] overflow-hidden [-webkit-line-clamp:2] [-webkit-box-orient:vertical] [display:-webkit-box]">
        ...
        {isDone && (taskCount || fileCount) ? (
          <div className="flex gap-1.5 flex-wrap items-center">
            {taskCount ? (
              <span className="text-[0.6rem] px-1.5 py-0.5 rounded-md bg-[var(--surface-raised)] border border-[var(--surface-border)] text-[var(--text-muted)] font-medium">
