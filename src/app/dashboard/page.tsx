...
const AgentMiniCard = ({ agent }: { agent: (typeof MOCK_AGENTS)[0] }) => (
  <Link href={`/dashboard/agents/${agent.id}`} className="no-underline">
    <div
      className={cn(
        glassCard,
        "p-4 flex items-center gap-3.5 transition-all duration-200 hover:border-[rgba(34,197,94,0.2)] cursor-pointer group",
      )}
    >
      {agent.avatar ? (
        <Image
      ...
      <div className="flex-1 min-w-0">
        ...
        <div className="flex items-center gap-1.5 mt-0.5">
          ...
          <span className="text-[0.78rem] text-[var(--text-muted)] capitalize">
            {agent.status}
          </span>
        </div>
      </div>
  ...
  return (
    <Link href="/dashboard/workspace" className="no-underline">
      <div
        className={cn(
          glassCard,
          "px-5 py-3.5 flex items-center gap-3.5 transition-all duration-200 hover:border-[rgba(34,197,94,0.15)] cursor-pointer group",
        )}
      >
        <div
          className={cn(
...
const RunsSkeleton = () => (
  <div className="flex flex-col gap-2.5">
    {Array.from({ length: 3 }).map((_, i) => (
      <div
        key={i}
        className={cn(glassCard, "px-5 py-3.5 flex items-center gap-3.5")}
      >
    ...
    <div className="p-4 sm:p-6 lg:p-10 max-w-[1200px] mx-auto space-y-6 sm:space-y-8">
      ...
      <div className="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-3">
        <div>
          <h1
            className={cn(
              "text-[1.75rem] sm:text-[2rem] font-bold font-[var(--font-display)] tracking-tight",
              textGradientTitle,
            )}
          >
            Command Center
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4">
        {[
          ...
          <div
            key={stat.label}
            className={cn(glassCard, "p-5 sm:p-6 flex items-start gap-4")}
          >
            <div
            ...
            <div className="min-w-0">
              ...
              <div
                className={cn(
                  "text-[1.4rem] sm:text-[1.75rem] font-bold font-[var(--font-display)] text-[var(--text)] tracking-tight mt-0.5",
                  loading && "animate-pulse",
                )}
              >
                {stat.value}
                {stat.suffix && (
                  <span className="text-[0.82rem] font-normal text-[var(--text-muted)]">
                    {stat.suffix}
                  </span>
                )}
              </div>
        ...
        <div className="lg:col-span-3 flex flex-col gap-4 sm:gap-6">
          ...
          <div>
            <div className="flex items-center justify-between mb-3">
              ...
              <Link
                href="/dashboard/workspace"
                className="text-[0.82rem] text-[var(--text-muted)] no-underline hover:text-[var(--text)] transition-colors"
              >
                View all &rarr;
              </Link>
            </div>
              ...
              >
                {data.recentActivity.map((item, i) => {
                  ...
                  const action = item.action
                    ...
                    .trim();
                  const actionShort =
                    action.length > 55 ? `${action.slice(0, 55)}…` : action;
                  const promptShort =
                    item.runPrompt.length > 48
                      ? `${item.runPrompt.slice(0, 48)}…`
                      : item.runPrompt;
                  return (
            ...
            <div className="grid grid-cols-2 gap-2.5">
              <Link href="/dashboard/agents" className="no-underline">
                <div
                  className={cn(
                    glassCard,
                    "p-4 flex flex-col items-center gap-2.5 text-center transition-all duration-200 hover:border-[rgba(167,139,250,0.25)] cursor-pointer group",
                  )}
                >
                  ...
                  <span className="text-[0.85rem] font-semibold text-[var(--text)] group-hover:text-[#a78bfa] transition-colors">
                    Hire Agent
                  </span>
                </div>
              </Link>
              <Link href="/dashboard/teams" className="no-underline">
                <div
                  className={cn(
                    glassCard,
                    "p-4 flex flex-col items-center gap-2.5 text-center transition-all duration-200 hover:border-[rgba(52,211,153,0.25)] cursor-pointer group",
                  )}
                >
                  ...
                  <span className="text-[0.85rem] font-semibold text-[var(--text)] group-hover:text-[#34d399] transition-colors">
                    Browse Teams
                  </span>
                </div>
              </Link>
