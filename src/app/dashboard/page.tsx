  ...
  return (
    <div className="p-4 sm:p-6 lg:p-10 max-w-[1200px] mx-auto space-y-6 sm:space-y-8">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4">
        {[
          { label: "Active Agents", value: MOCK_STATS.activeAgents, suffix: `/${MOCK_AGENTS.length}`, color: "#22c55e", icon: <svg width={22} height={22} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8"><path d="M12 8V4H8" /><rect x="4" y="8" width="16" height="12" rx="2" /></svg> },
          { label: "Tasks Done", value: MOCK_STATS.tasksCompleted.toLocaleString(), suffix: "", color: "#3b82f6", icon: <svg width={22} height={22} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8"><polyline points="9 11 12 14 22 4" /><path d="M21 12v7a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h11" /></svg> },
          { label: "Teams", value: MOCK_STATS.activeTeams, suffix: " active", color: "#a78bfa", icon: <svg width={22} height={22} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8"><path d="M16 21v-2a4 4 0 0 0-4-4H6a4 4 0 0 0-4 4v2" /><circle cx="9" cy="7" r="4" /><path d="M22 21v-2a4 4 0 0 0-3-3.87" /><path d="M16 3.13a4 4 0 0 1 0 7.75" /></svg> },
          { label: "Uptime", value: MOCK_STATS.uptime, suffix: "", color: "#f59e0b", icon: <svg width={22} height={22} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8"><path d="M22 12h-4l-3 9L9 3l-3 9H2" /></svg> },
        ].map((stat) => (
          <div key={stat.label} className={cn(glassCard, "p-5 sm:p-6 flex items-start gap-4")}>
            ...
            <div className="min-w-0">
              ...
              <div className="text-[1.4rem] sm:text-[1.75rem] font-bold font-[var(--font-display)] text-[var(--text)] tracking-tight mt-0.5">
                {stat.value}
          ...
          <div>
            ...
            <div className={cn(glassCard, "divide-y divide-[var(--surface-border)] overflow-hidden")}>
              {ACTIVITY_FEED.map((item, i) => (
                <div key={i} className="px-5 py-3.5 flex items-center gap-3 text-[0.88rem]">
                  <span
                  ...
                  <span className="flex-1 min-w-0">
                    ...
                    {" "}
                    <span className="font-mono text-[0.82rem] text-[#22c55e]">{item.target}</span>
                  </span>
                  <span className="text-[0.75rem] text-[var(--text-muted)] shrink-0">{item.time}</span>
                </div>
              ))}
            </div>
          </div>
