...
const MAX_STALE_POLLS = 100; // ~5 min of silence → give up (agents can take 30s+ between events)
    ...
    const poll = async () => {
      ...
      if (currentEventCount === lastEventCountRef.current) {
        ...
        if (staleCountRef.current >= MAX_STALE_POLLS) {
          // Pipeline is likely dead — force done so the UI isn't stuck
          const { history: finalHistory, allTasks: finalTasks } = replayWorkflowRuns([run]);
          if (finalHistory[0]) {
            const staleEntry = finalHistory[0];
            const stuckAgents = staleEntry.workingAgents;
            onUpdateRef.current({
              entry: {
                ...staleEntry,
                done: true,
                workingAgents: [],
                error: staleEntry.error ?? (stuckAgents.length > 0
                  ? `Pipeline stalled. Agents stopped responding: ${stuckAgents.join(", ")}.`
                  : null),
              },
              tasks: finalTasks,
            });
